...
def get_range_hint(number, current_min=1, current_max=100):
    ...
    return f"HINT: the narrowed range around the number is from {max(number - 12, current_min)} to {min(number + 12, current_max)}"
...
random_number = random.randint(1, 100)  # แก้ชื่อและฟังก์ชันผิด
attempt = 1
...
while True:
    ...
    if random_number == guess_number:
        print(f"WP you won in {attempt} Attempt(s)")
        break
